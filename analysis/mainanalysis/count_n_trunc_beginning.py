...
for eachParti in range(5,7):
    ...
    for eachRun in range (1,13):
        ...
        beginning = data.find("imagine phase")
        #beginning = data.find("trial begins. trialTime reset")
        print(task_name, eachRun)
        print(beginning)
        
        #get number of occurrences of the substring in the string
        n_Trig = data.count("Trigger received")
        
        #get number of occurrences of the substring in the string
        n_trig = data.count("trigger received")
        
        total_trig = n_Trig + n_trig
    ...
    #np.savetxt(path+'/data/behavioral/data_10_15_21/n_trunc_beginning/n_trunc_beginning_'+sub+'.csv', partiData, fmt='%f', delimiter=",", header = 'run,triggerCount')
