...
# In[44]:
        ...
        out_dir_sub= 'derivatives/firstlevel/%s/' % (sub)
        out_dir_ses= '%s/' % (ses)
        out_dir= adderzip_bids_dir + out_dir_sub + out_dir_ses
        mask_fold_new = 'derivatives/firstlevel/%s/masks/' % sub
        mask_fold = adderzip_bids_dir + mask_fold_new
        ...
        if os.path.exists(out_dir)==False:
            ...
            path = os.path.join(adderzip_bids_dir, out_dir_sub)
            os.mkdir(path) 
            path = os.path.join(adderzip_bids_dir+out_dir_sub,out_dir_ses)
            os.mkdir(path)
        if os.path.exists(mask_fold)==False:
            print('making new directory:', mask_fold)
            path = os.path.join(adderzip_bids_dir,mask_fold_new)
            os.mkdir(path)
        ...
        confounds_selected.to_csv(out_dir + sub + '_ses-01_task-localizer_run-0%i_confounds_selected_trim%dandEndTRs.txt' % (run, n_trunc_beginning), index=False, sep='\t', mode='w')
